...
export const CompositeModal: React.FC<CompositeModalProps> = ({
  ...
  const handleGenerate = () => {
    onGenerate(Array.from(selectedIds), config.prompt, { format: config.format, resolution: config.resolution, aspectRatio: config.aspectRatio });
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
          ...
          <div className="mb-6 bg-slate-900/50 p-4 rounded-xl border border-slate-800">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="space-y-1">
                ...
                <select
                  value={config.format}
                  onChange={(e) => onConfigChange({ ...config, format: e.target.value as OutputFormat })}
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-300 focus:ring-2 focus:ring-emerald-500/50 outline-none"
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => onConfigChange({ ...config, resolution: e.target.value as AiResolution })}
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-300 focus:ring-2 focus:ring-emerald-500/50 outline-none"
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              placeholder={t('compositePrompt')}
              className="w-full h-24 bg-slate-900 border border-slate-700 rounded-xl p-4 text-sm text-slate-300 focus:ring-2 focus:ring-emerald-500/50 outline-none resize-none"
            />
          </div>
        </div>
        ...
        <div className="p-6 border-t border-slate-800 bg-[#020617] flex justify-end gap-3">
          ...
          <button
            onClick={() => onGenerate(Array.from(selectedIds), config.prompt, { format: config.format, resolution: config.resolution, aspectRatio: config.aspectRatio })}
            disabled={selectedIds.size < 2 || !config.prompt}
            className="px-6 py-2 bg-gradient-to-r from-emerald-500 to-cyan-500 text-white font-medium rounded-xl hover:shadow-lg hover:shadow-emerald-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95 flex items-center gap-2"
          >
